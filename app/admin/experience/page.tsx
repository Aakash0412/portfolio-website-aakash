          ...
          <Table>
            <TableHeader style={{ background: 'rgba(255, 255, 255, 0.02)' }}>
              <TableRow style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
                <TableHead style={{ color: 'var(--text-muted)', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.05em', height: '56px', paddingLeft: '24px' }}>Role</TableHead>
                <TableHead style={{ color: 'var(--text-muted)', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.05em', height: '56px' }}>Organization</TableHead>
            ...
            <TableBody>
              {experience.map((exp) => (
                <TableRow key={exp.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)', transition: 'all 0.3s ease' }} className="hover:bg-white/[0.03]">
                  <TableCell style={{ fontWeight: '500', fontSize: '15px', color: 'var(--fg)', padding: '20px 24px' }}>{exp.role}</TableCell>
                  <TableCell style={{ padding: '20px 16px' }}>{exp.organization}</TableCell>
                  <TableCell style={{ padding: '20px 16px', color: 'var(--text-muted)' }}>{exp.startDate} - {exp.endDate || (exp.isCurrent ? 'Present' : '')}</TableCell>
                  <TableCell style={{ padding: '20px 16px' }}>
                    <span style={{ 
                      ...
                      fontSize: '12px', 
                      fontWeight: '500',
                      background: exp.isPublished ? 'rgba(46, 213, 115, 0.1)' : 'rgba(255, 71, 87, 0.1)',
                      color: exp.isPublished ? '#2ed573' : '#ff4757',
                      border: `1px solid ${exp.isPublished ? 'rgba(46, 213, 115, 0.2)' : 'rgba(255, 71, 87, 0.2)'}`
                    }}>
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: exp.isPublished ? '#2ed573' : '#ff4757', boxShadow: `0 0 8px ${exp.isPublished ? '#2ed573' : '#ff4757'}` }}></span>
                      {exp.isPublished ? 'Public' : 'Hidden'}
                    </span>
                  </TableCell>
