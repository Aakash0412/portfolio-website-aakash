    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '16px', color: 'var(--fg)', padding: '0', overflow: 'hidden' }} className="sm:max-w-[425px]">
        <div style={{ padding: '24px', borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
          <DialogTitle style={{ fontSize: '1.25rem' }}>{isEditing ? "Edit Achievement" : "New Achievement"}</DialogTitle>
        </div>
        <form onSubmit={handleSubmit} className="contact-form" style={{ padding: '24px', background: 'transparent', border: 'none', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="form-group">
            ...
            <Input id="title" name="title" defaultValue={achievement?.title} required className="form-input" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="form-group">
              <Label htmlFor="date" className="form-label">Date</Label>
              <Input id="date" name="date" defaultValue={achievement?.date || ""} className="form-input" placeholder="e.g. 2023" />
            </div>
            <div className="form-group">
              <Label htmlFor="url" className="form-label">URL (Optional)</Label>
              <Input id="url" name="url" defaultValue={achievement?.url || ""} className="form-input" placeholder="https://..." />
            </div>
          </div>
          <div className="form-group">
            <Label htmlFor="description" className="form-label">Description</Label>
            <textarea id="description" name="description" defaultValue={achievement?.description || ""} rows={3} className="form-input" />
          </div>
          
          <div className="flex items-center space-x-3 mt-2">
            <input type="checkbox" id="isPublished" name="isPublished" value="true" defaultChecked={achievement?.isPublished} style={{ accentColor: 'var(--fg)', width: '16px', height: '16px' }} />
